...
int main()
{
    ...
    ///1-define struct
    struct process {int id, at, bt, wt, p;};
    ...
    ///5-Sorting Process in ascending order according to its arrival time and Priority using Bubble Sort
    for(int i=0 ; i<n-1 ; i++){
        for(int j=i+1 ; j<n ; j++){
    ...
    arr[0].wt=0;
    for(int i=1 ; i<n ; i++){
        arr[i].wt=arr[i-1].wt+arr[i-1].bt; /// wt[i]=wt[i-1]+bt[i-1];
    }
    cout<<"---------------------------------------"<<endl;

    ///7-The Sequence of Processes Will be & The Waiting Time for each process
    cout<<"Process     Arrival time     Burst Time     Priority     Waiting Time"<<endl;
    for(int i=0 ; i<n ;i++){
        cout<<"P"<<arr[i].id<<"\t \t"<<arr[i].at<<"\t \t"<<arr[i].bt<<"\t \t"<<arr[i].p<<"\t \t"<<arr[i].wt<<endl;
    }
    cout<<"---------------------------------------"<<endl;
